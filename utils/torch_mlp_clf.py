...
class TorchMLPClassifier:
    ...
    def score(self, test_X, test_y, device = torch.device('cuda' if torch.cuda.is_available() else 'cpu'), logger=None):
    ...
    def predict(self, X, device = torch.device('cuda' if torch.cuda.is_available() else 'cpu'), multi_label_n_class=None, logger=None):
        logger = logger if logger else logging.getLogger(__name__)
